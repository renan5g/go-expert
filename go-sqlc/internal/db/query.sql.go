...
const findCategoryByCourseId = `-- name: FindCategoryByCourseId :one
SELECT c.id, c.name, c.description 
FROM categories c JOIN courses co ON c.id = co.category_id 
WHERE co.id = ?
`

func (q *Queries) FindCategoryByCourseId(ctx context.Context, id string) (Category, error) {
	row := q.db.QueryRowContext(ctx, findCategoryByCourseId, id)
	var i Category
