...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Register() {
	...
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handles[suite.event.GetName()]))

	err = suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler2)
	suite.Nil(err)
	suite.Equal(2, len(suite.eventDispatcher.handles[suite.event.GetName()]))

	assert.Equal(suite.T(), &suite.handler, suite.eventDispatcher.handles[suite.event.GetName()][0])
	assert.Equal(suite.T(), &suite.handler2, suite.eventDispatcher.handles[suite.event.GetName()][1])
}
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Clear() {
	...
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handles[suite.event2.GetName()]))

	suite.eventDispatcher.Clear()
	suite.Equal(0, len(suite.eventDispatcher.handles))
}
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Remove() {
	...
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handles[suite.event.GetName()]))
	assert.Equal(suite.T(), &suite.handler2, suite.eventDispatcher.handles[suite.event.GetName()][0])

	err = suite.eventDispatcher.Remove(suite.event.GetName(), &suite.handler2)
	suite.Nil(err)
	suite.Equal(0, len(suite.eventDispatcher.handles[suite.event.GetName()]))
