...
func TestCreateProduct(t *testing.T) {
	...
	p, _ := entity.NewProduct("Product 1", 10.0)
	...
	err = db.First(&productFound, "id = ?", p.ID).Error
	assert.Nil(t, err)
	assert.Equal(t, p.ID, productFound.ID)
	assert.Equal(t, p.Name, productFound.Name)
	assert.Equal(t, p.Price, productFound.Price)
}
